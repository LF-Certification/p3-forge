...
FONT_SIZE=$(echo "$TERMINAL_CONFIG" | jq -r '.fontSize // "16"')
...
exec ttyd -I /usr/share/ttyd/index.html -t fontSize=$FONT_SIZE -t macOptionClickForcesSelection=true -t disableLeaveAlert=true -W tmux new-session -A -s remote "while true; do $SSH_COMMAND || echo \"SSH failed with exit code \$?\"; echo \"Reconnecting in $RETRY_INTERVAL seconds...\"; sleep $RETRY_INTERVAL; done"
